	...
	if typeCode == ScanTypeCode && responseMode == ContinuousResponse {
		...
		for {
			...
			default:
				...
				// if numScanPacketHeaderBytes != 10, log the actual value
				if numScanPacketHeaderBytes != scanPacketHeaderSize {
					log.Printf("The lidar gave us %v in the header packet. Expected 10.", numScanPacketHeaderBytes)
					log.Printf("The header packet is: %X ", scanPacketHeader)
					continue
				}
				...
				packetHeader, scanningFrequency, dataPacketType, sampleQuantity := lidar.extractScanPacketHeader(pointCloud)

				log.Printf("Chars: %X", packetHeader)

				if packetHeader == 0 && scanningFrequency == 0 && dataPacketType == 0 && sampleQuantity == 0 {
					log.Printf("O packet as fuck")
					continue
				}
				...
				case 0x1:
					...
					if numScanPacketHeaderBytes != scanPacketHeaderSize {
						log.Printf("not enough theBytes in header. Expected %v got %v", scanPacketHeaderSize, numScanPacketHeaderBytes)
					}
					if sampleQuantity != 1 {
				...
				case 0x0:
					...
					// 3 theBytes per sample, ex. If sampleQuantity is 5, then numOfSamples is 15 because there are 5 samples and each sample is 3 theBytes.
					...
					// Check if the number of theBytes read is equal to the number of samples.
					if numScanPacketHeaderBytes != numOfSamples {
						log.Print(fmt.Errorf("start Scan Sampling Quality: not enough theBytes. Expected %v got %v", numOfSamples, numScanPacketHeaderBytes))
					}
					...
					n := 3
					splitReadings := make([][]byte, len(theBytes)/n)

					intensities := make([]uint16, len(theBytes)/n)
					distances := make([]uint16, len(theBytes)/n)
					for Si := range splitReadings {
						splitReadings[Si] = theBytes[Si*n : (Si+1)*n]
...
func (lidar *YDLidar) extractScanPacketHeader(pointCloud pointCloudHeader) (uint16, uint8, uint8, uint8) {
	...
	scanningFrequency := (pointCloud.PackageType >> 1) / 10
...
func checkScanPacket(scanPacketHeaderSlice []byte, scanPointCloudSlice []byte) error {
	...
	C1 := make([]uint16, 1)
	bufferedC1 := bytes.NewBuffer(scanPacketHeaderSlice[0:2])
	...
	C2 := make([]uint16, 1)
	bufferedC2 := bytes.NewBuffer(scanPacketHeaderSlice[4:6])
	...
	bufferedNextToLastC := bytes.NewBuffer(scanPacketHeaderSlice[2:4])
	...
	lastC := make([]uint16, 1)
	bufferedLastC := bytes.NewBuffer(scanPacketHeaderSlice[6:8])
	...
	samplePacket := make([]uint16, len(scanPointCloudSlice)/3)
	bufferedSamples := bytes.NewBuffer(scanPointCloudSlice)
	...
	//if checkCode != scanPacketHeaderSlice[8] {
